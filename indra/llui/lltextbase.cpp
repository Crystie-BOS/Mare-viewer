...
// [/SL:KB]
            ...
            // Use F32 otherwise a string of multiple segments
            // will accumulate a large error
            F32 left_precise = static_cast<F32>(line_iter->mRect.mLeft);
            F32 right_precise = static_cast<F32>(line_iter->mRect.mLeft);
...
//                          break;
            ...
            LLRect selection_rect;
            selection_rect.mLeft = static_cast<S32>(left_precise);
            selection_rect.mRight = static_cast<S32>(right_precise);
            selection_rect.mBottom = line_iter->mRect.mBottom;
            selection_rect.mTop = line_iter->mRect.mTop;
...
//virtual
void LLTextBase::setFocusColor(const LLUIColor &c)
{
    mFocusBgColor = c;
    mStyleDirty = true;
}
...
//virtual
void LLTextBase::setWriteableColor(const LLUIColor &c)
{
    mWriteableBgColor = c;
    mStyleDirty = true;
}
...
void LLTextBase::refreshHighlights()
{
    if (mHighlightsDirty)
    {
        ...
        if (!mHighlightWord.empty())
        {
            ...
            {
                S32 idxStart = static_cast<S32>(itRange->begin() - wstrText.begin());
                mHighlights.push_back(range_pair_t(idxStart, static_cast<S32>(idxStart + itRange->size())));
            }
        }
        mHighlightsDirty = false;
    }
