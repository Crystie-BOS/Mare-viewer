...
//mk
		...
		else
		{
			...
			case CHAT_TYPE_WHISPER:
				...
				break;
//MK
				// We should not treat debug and owner chat messages the same way in the RLV
				// or we open a security loophole.
...
void process_kill_object(LLMessageSystem *mesgsys, void **user_data)
{
	...
	for (S32 i = 0; i < num_objects; ++i)
	{
			...
			LLVOAvatar::findAvatarFromAttachment(objectp) == gAgentAvatarp)
		{
			...
			{
				LL_INFOS("Attachment") << "Received kill-object message from non-agent region for agent attachment: "
					<< objectp->getAttachmentItemName() << ". Ignoring."
					<< LL_ENDL;
			...
			LL_INFOS("Attachment") << "Received kill object order for agent attachment: "
				<< objectp->getAttachmentItemName()
...
//CA Imported code from Henri Beauchamp ends above this comment
