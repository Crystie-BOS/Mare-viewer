...
bool    LLPanelFace::postBuild()
{
    ...
    getChildSetCommitCallback(mTexScaleFlipU, "TexScaleFlipU", [&](LLUICtrl*, const LLSD&) { onCommitTextureInfo(); });
    getChildSetCommitCallback(mTexScaleFlipV, "TexScaleFlipV", [&](LLUICtrl*, const LLSD&) { onCommitTextureInfo(); });
    ...
    getChildSetCommitCallback(mBumpyScaleFlipU, "bumpyScaleFlipU", [&](LLUICtrl*, const LLSD&) { onCommitMaterialBumpyScaleX(); });
    getChildSetCommitCallback(mBumpyScaleFlipV, "bumpyScaleFlipV", [&](LLUICtrl*, const LLSD&) { onCommitMaterialBumpyScaleY(); });
    ...
    getChildSetCommitCallback(mShinyScaleFlipU,"shinyScaleFlipU", [&](LLUICtrl*, const LLSD&) { onCommitMaterialShinyScaleX(); });
    getChildSetCommitCallback(mShinyScaleFlipV,"shinyScaleFlipV", [&](LLUICtrl*, const LLSD&) { onCommitMaterialShinyScaleY(); });
...
{ //set state of UI to match state of texture entry(ies)  (calls setEnabled, setValue, etc, but NOT setVisible)
        ...
        && objectp->permModify())
    {
        ...
        // Texture scale
        {
            ...
            mTexScaleFlipU->setEnabled(editable);
            mShinyScaleFlipU->setEnabled(editable && specmap_id.notNull());
            mBumpyScaleFlipU->setEnabled(editable && normmap_id.notNull());
            ...
            mTexScaleFlipU->setTentative(LLSD(diff_scale_tentative));
            mShinyScaleFlipU->setTentative(LLSD(spec_scale_tentative));
            mBumpyScaleFlipU->setTentative(LLSD(norm_scale_tentative));
        }

        {
            ...
            mTexScaleFlipV->setEnabled(editable);
            mShinyScaleFlipV->setEnabled(editable && specmap_id.notNull());
            mBumpyScaleFlipV->setEnabled(editable && normmap_id.notNull());
            if (force_set_values)
            {
            ...
            mBumpyScaleFlipV->setValue(spec_scale_flip_t);
            ...
            mTexScaleFlipV->setTentative(LLSD(diff_scale_tentative));
            mShinyScaleFlipV->setTentative(LLSD(spec_scale_tentative));
            mBumpyScaleFlipV->setTentative(LLSD(norm_scale_tentative));
        }
        ...
        // Texture offset
        {
            ...
            mTexDuplicate->setEnabled(editable && (normmap_id.notNull() || specmap_id.notNull()));
            mShinyDuplicate->setEnabled(editable && specmap_id.notNull());
            mBumpyDuplicate->setEnabled(editable && normmap_id.notNull());
        }
        ...
        // Materials
        {
            ...
            if (material && editable)
            {
                ...
                if (!material->getSpecularID().isNull() && (shiny == SHINY_TEXTURE))
                {
                    ...
                    mShinyScaleFlipU->setValue(flip_x);
                    mShinyScaleFlipV->setValue(flip_y);
                ...
                if (!material->getNormalID().isNull())
                {
                    ...
                    mBumpyScaleFlipU->setValue(flip_x);
                    mBumpyScaleFlipV->setValue(flip_y);
        ...
        if (node->isTESelected(te))
        {
            ...
            if (tep)
            {
                ...
                if (te_data["te"].has("imageid") || pbr_id.notNull())
                {
                    ...
                    // unused bool pbr_from_library = false;
                    // unused bool pbr_full_perm = false;
...
void LLPanelFace::onClickDuplicateDiffuse()
{
    ...
    //
    F32 value = (F32)mTexScaleU->getValue().asReal();
    BOOL flip = mTexScaleFlipU->getValue().asBoolean();

    if (have_normal_map) {
        mBumpyScaleU->setValue(value);
        mBumpyScaleFlipU->setValue(flip);
        LLSelectedTEMaterial::setNormalRepeatX(this, flip ? -value : value);
    }
    if (have_specular_map) {
        mShinyScaleU->setValue(value);
        mShinyScaleFlipU->setValue(flip);
        LLSelectedTEMaterial::setSpecularRepeatX(this, flip ? -value : value);
    }

    //
    //  duplicate diffuse map scale (V)
    //
    value = (F32)mTexScaleV->getValue().asReal();
    flip = mTexScaleFlipV->getValue().asBoolean();

    if (have_normal_map) {
        mBumpyScaleV->setValue(value);
        mBumpyScaleFlipV->setValue(flip);
        LLSelectedTEMaterial::setNormalRepeatY(this, flip ? -value : value);
    }
    if (have_specular_map) {
        mShinyScaleV->setValue(value);
        mShinyScaleFlipV->setValue(flip);
        LLSelectedTEMaterial::setSpecularRepeatY(this, flip ? -value : value);
    }

    //
    //  duplicate diffuse map rotation
    //
    value = (F32)mTexRotate->getValue().asReal();

    if (have_normal_map) {
        mBumpyRotate->setValue(value);
        LLSelectedTEMaterial::setNormalRotation(this, value * DEG_TO_RAD);
    }
    if (have_specular_map) {
        mShinyRotate->setValue(value);
        LLSelectedTEMaterial::setSpecularRotation(this, value * DEG_TO_RAD);
    }

    //
    //  duplicate diffuse map offset (U)
    //
    value = (F32)mTexOffsetU->getValue().asReal();

    if (have_normal_map) {
        mBumpyOffsetU->setValue(value);
        LLSelectedTEMaterial::setNormalOffsetX(this, value);
    }
    if (have_specular_map) {
        mShinyOffsetU->setValue(value);
        LLSelectedTEMaterial::setSpecularOffsetX(this, value);
    }

    //
    //  duplicate diffuse map offset (V)
    //
    value = (F32)mTexOffsetV->getValue().asReal();

    if (have_normal_map) {
        mBumpyOffsetV->setValue(value);
        LLSelectedTEMaterial::setNormalOffsetY(this, value);
    }
    if (have_specular_map) {
        mShinyOffsetV->setValue(value);
        LLSelectedTEMaterial::setSpecularOffsetY(this, value);
    }
}
...
void LLPanelFace::onClickDuplicateNormal()
{
    ...
    //
    F32 value = (F32)mBumpyScaleU->getValue().asReal();
    BOOL flip = mBumpyScaleFlipU->getValue().asBoolean();

    mTexScaleU->setValue(value);
    mTexScaleFlipU->setValue(flip);
    ...
    //
    value = (F32)mBumpyScaleV->getValue().asReal();
    flip = mBumpyScaleFlipV->getValue().asBoolean();

    mTexScaleV->setValue(value);
    mTexScaleFlipV->setValue(flip);
    ...
    //
    value = (F32)mBumpyRotate->getValue().asReal();
    mTexRotate->setValue(value);
    ...
    //
    value = (F32)mBumpyOffsetU->getValue().asReal();
    mTexOffsetU->setValue(value);
    ...
    //
    value = (F32)mBumpyOffsetV->getValue().asReal();
    mTexOffsetV->setValue(value);
...
void LLPanelFace::onClickDuplicateSpecular()
{
    ...
    //
    F32 value = (F32)mShinyScaleU->getValue().asReal();
    F32 flip = mShinyScaleFlipU->getValue().asBoolean();
    ...
    //
    value = (F32)mShinyScaleV->getValue().asReal();
    flip = mShinyScaleFlipV->getValue().asBoolean();
    ...
    //
    value = (F32)mShinyRotate->getValue().asReal();
    mTexRotate->setValue(value);
    ...
    //
    value = (F32)mShinyOffsetU->getValue().asReal();
    mTexOffsetU->setValue(value);
    ...
    //
    value = (F32)mShinyOffsetV->getValue().asReal();
    mTexOffsetV->setValue(value);
