...
void LLInventoryGalleryContextMenu::updateMenuItemsVisibility(LLContextMenu* menu)
{
    ...
    if(is_folder)
    {
        ...
        if (selected_category)
        {
            ...
            is_system_folder = LLFolderType::lookupIsProtectedType(folder_type, selected_id);
    ...
    else
    {
        ...
        if (LLClipboard::instance().hasContents() && is_agent_inventory && !is_cof && !is_inbox)
        {
            ...
            if (inventory_linking)
            {
                ...
                else if (selected_category && LLFolderType::lookupIsProtectedType(selected_category->getPreferredType(), selected_id))
                {
                    disabled_items.push_back(std::string("Paste As Link"));
                }
            }
        ...
        if (marketplacelistings_id.notNull() && !is_inbox && !obj->getIsLinkType())
        {
            if (is_folder)
            {
                if (selected_category
                    ...
                    && !LLFolderType::lookupIsProtectedType(selected_category->getPreferredType(), selected_id)
