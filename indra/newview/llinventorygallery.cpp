...
void LLInventoryGalleryItem::setType(LLAssetType::EType type, LLInventoryType::EType inventory_type, U32 flags, bool is_link)
{
    ...
    if(mIsFolder)
    {
        ...
        if (cat)
        {
            ...
            else if(LLFolderType::lookupIsProtectedType(cat->getPreferredType(), folder_id))
            {
    ...
    if(hasMouseCapture())
    {
        ...
        if(LLToolDragAndDrop::getInstance()->isOverThreshold(screen_x, screen_y))
        {
            ...
            if(cat && gInventory.isObjectDescendentOf(mUUID, gInventory.getRootFolderID())
                   && !LLFolderType::lookupIsProtectedType((cat)->getPreferredType(), mUUID))
            {
    ...
    if(LLToolDragAndDrop::SOURCE_AGENT == source)
    {
        ...
        switch (inv_item->getActualType())
        {
            case LLAssetType::AT_CATEGORY:
                is_movable = !LLFolderType::lookupIsProtectedType(((LLInventoryCategory*)inv_item)->getPreferredType(), folder_id);
                break;
            default:
                break;
        }
                            ...
                            BOOL drop, std::string& tooltip_msg, BOOL is_link)
{
    ...
    if (is_agent_inventory)
    {
        ...
        if (is_movable && LLFolderType::lookupIsProtectedType(inv_cat->getPreferredType(), dest_id))
        {
            is_movable = FALSE;
            // tooltip?
        }
        ...
        if (is_movable)
        {
            ...
            for (S32 i=0; i < descendent_categories.size(); ++i)
            {
                ...
                if(LLFolderType::lookupIsProtectedType(category->getPreferredType(), cat_id))
                {
