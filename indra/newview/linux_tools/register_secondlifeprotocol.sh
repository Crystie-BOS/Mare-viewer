...
if which ${LLGCONFTOOL2} >/dev/null; then
    (${LLGCONFTOOL2} write /desktop/gnome/url-handlers/secondlife/command "'${HANDLER} \"%s\"'" && ${LLGCONFTOOL2} write /desktop/gnome/url-handlers/secondlife/enabled true) || echo Warning: Did not register hop:// handler with GNOME: ${LLGCONFTOOL2} failed.
else
