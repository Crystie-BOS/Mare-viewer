...
//MK
                ...
                for (unsigned int index = 0; index < MAX_CLOTHING_LAYERS; ++index)
                {
                    ...
                    S32 count = static_cast<S32>(wearables.size());
                    llassert(items.size() == count);
                    S32 i;
                    for (i = 0; i < count; i++)
                    {
...
//MK
    ...
    if (gRRenabled)
    {
        if (gAgentAvatarp && !gAgentAvatarp->getHasMissingParts() && !gAgent.mRRInterface.mUserUpdateAttachmentsFirstCall && !gAgent.mRRInterface.mUserUpdateAttachmentsCalledManually)
        {
...
//mk
    ...
    S32 remove_count = (S32)objects_to_remove.size();
    S32 add_count = (S32)items_to_add.size();
